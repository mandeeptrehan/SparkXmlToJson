...
object Xml2Json {
  ...
  def main(args: Array[String]) {
    //UDF to convert xml value to string
    ...
    val inputDF = spark.readStream.text(path = "file:///Users/mbawa/Downloads/xml/");

    val jsonDF = inputDF.select(from_json(parse(col("value")), schema).as("Event")).select("Event.Event.*")
